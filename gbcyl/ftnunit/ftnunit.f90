...
subroutine test( proc, text )
    ...
    !
    testno = testno + 1
    !! Commented out by Jouni Karjalainen, since calling runtests multiple 
    !! times resulted in the first test not being performed.
...
!     Subroutine to check if two doubles are approximately equal
! Arguments:
!     value1        First value
!     value2        Second value
!     margin        Allowed margin (relative)
...
subroutine assert_comparable_double( value1, value2, margin, text )
    real(doublep), intent(in)             :: value1
    real(doublep), intent(in)             :: value2
    real(doublep), intent(in)             :: margin
    character(len=*), intent(in) :: text
...
! assert_compatable_double1d --
!     Subroutine to check if two double arrays are comparable
! Arguments:
!     array1        First array
!     array2        Second array
!     text          Text describing the assertion
...
subroutine assert_comparable_double1d( array1, array2, margin, text )
    real(doublep), dimension(:), intent(in)    :: array1
    real(doublep), dimension(:), intent(in)    :: array2
    real(doublep), intent(in)                  :: margin
    character(len=*), intent(in)      :: text
