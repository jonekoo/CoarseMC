...
!! Computes all eigenvalues and eigenvectors of a real symmetric N ×N matrix a.
!! On output, elements of a above the diagonal are destroyed. d is a vector of
!! length N that returns the eigenvalues of a. v is an N×N matrix whose 
...
d(:)=b(:) 
z(:)=0.0 !! This vector will accumulate terms of the form ta_pq as in eq. 
         !! (11.1.14). 
nrot=0 
do i=1,50 
  sm=sum(abs(a),mask=upper_triangle(n,n)) !! Sum oﬀ-diagonal elements. 
  if(sm==0.0)RETURN 
  !! The normal return, which relies on quadratic convergence to machine 
  !! underﬂow. 
  tresh=merge(0.2_sp*sm/n**2,0.0_sp,i<4 ) 
  !! On the ﬁrst three sweeps, we will rotate only if tresh exceeded. 
  do ip=1,n-1 
    do iq=ip+1,n 
      g=100.0_sp*abs(a(ip,iq)) 
      !! After four sweeps, skip the rotation if the oﬀ-diagonal element is 
      !! small. 
  ...
  enddo 
  b(:)=b(:)+z(:) 
  d(:)=b(:) !! Update d with the sum of ta_pq, 
