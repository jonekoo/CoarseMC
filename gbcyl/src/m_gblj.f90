...
module m_gblj
  ...
  use m_rodpoint_interaction, only: rodpoint_potential
  use m_rod
  use m_point
  implicit none
  ...
  type, extends(rodpoint_potential) :: gblj_potential
   ...
   contains
     ...
     procedure :: potential => gblj_potential_potential
     procedure :: gblj_value
...
contains
  ...
  end function gblj_readparameters
  ...
  end subroutine gblj_init_internal
  ...
  end subroutine gblj_writeparameters
    
  subroutine gblj_potential_potential(this, particle_i, particle_j, &
       res, err)
    class(gblj_potential), intent(in) :: this
    type(rod), intent(in) :: particle_i
    type(point), intent(in) :: particle_j
    real(dp), intent(out) :: res
    integer, intent(out) :: err
    logical :: overlap
    call this%gblj_value(particle_i%orientation(), &
         particle_j%position() - particle_i%position(), res, overlap)
    if (overlap) err = -1
  end subroutine gblj_potential_potential
  ...
  subroutine test_defaults(this)
    ...
    do 
       ...
       if (ios /= 0) exit
       call this%gblj_value(ui, rij, energy, overlap)
