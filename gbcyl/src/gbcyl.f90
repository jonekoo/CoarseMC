...
program gbcyl
  ...
  character(len=32) :: restart = 'no'
  integer :: n_equilibration_sweeps = -1
  integer :: n_production_sweeps = -1
  namelist /start/ restart, n_production_sweeps, n_equilibration_sweeps
!!  integer :: n_args
...
!!  end if
  read(*, nml = start)
  if (restart == 'yes') then
    if (0 .le. n_equilibration_sweeps) &
      call equilibration_sweeps(n_equilibration_sweeps)
    if (0 .le. n_production_sweeps) &
      call production_sweeps(n_production_sweeps) 
  else
    call init
  end if 
