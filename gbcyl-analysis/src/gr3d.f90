...
subroutine gr3d(particles, N, Lx, Ly, Lz, x_periodic, y_periodic, z_periodic,& 
  ...
  do i = 1, N-1
     do j = i+1, N
           ...
           if(x_periodic) dx = dx - nint(dx/Lx)*Lx
           if(y_periodic) dy = dy - nint(dy/Ly)*Ly
           if(z_periodic) dz = dz - nint(dz/Lz)*Lz
           
           r = sqrt(dx*dx + dy*dy + dz*dz)
           bin = int(r/delr) + 1
           if(bin <= maxbin) histogram(bin) = histogram(bin) + 2
     end do
  end do

  density = real(N)/(Lx*Ly*Lz) 
  !! Normalize by n_ideal_gas = bin_volume*ideal_gas_density and N
  do bin = 1, maxbin
     rlow = real(bin-1)*delr
     n_ideal_gas = 4.0*pi*density/3.0*((rlow + delr)**3-rlow**3)
     histogram(bin) = histogram(bin)/real(N)/n_ideal_gas
  end do
